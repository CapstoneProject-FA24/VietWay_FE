...
import { Box, Typography, Grid, TextField, Button, Select, MenuItem, FormControl, InputLabel, Checkbox, FormControlLabel, Divider } from '@mui/material';
...
const BookTour = () => {
  ...
  const calculateTotal = () => {
    ...
    const { adultPrice, childPrice, infantPrice, babyPrice, singleRoomSurcharge, adultOnlineDiscount, childOnlineDiscount } = bookingData.bookingInfo;
    return (
    ...
    <Box sx={{ 
      ...
      <ContentContainer>
        <StyledBox>
          ...
          <Typography variant="h4" align="center" style={{ fontWeight: 'bolder', fontSize: 45, marginBottom: 30, marginTop: 40, color: '#3572EF' }} gutterBottom>ĐẶT TOUR</Typography>
          <StepBox>
          ...
          <Grid container spacing={3} sx={{ maxWidth: '100%' }}>
            <Grid item xs={12} md={8} sx={{ maxWidth: '100%' }}>
              ...
              {[...Array(formData.infants)].map((_, index) => (
                <PassengerInfo key={`infants-${index}`}>
                  <Typography variant="subtitle1" style={{ fontWeight: 'bold' }}>Em bé (Dưới 2 tuổi)</Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                      <CustomTextField fullWidth label="Họ tên *" />
                    </Grid>
            ...
            <Grid item xs={12} md={4} sx={{ maxWidth: '100%' }}>
              <SummaryTitle variant="h5" style={{ alignContent: 'center' }}>TÓM TẮT THÔNG TIN CHUYẾN ĐI</SummaryTitle>
              <SummaryBox>
                <Box sx={{ mb: 2 }}>
                  <img src={bookingData.images?.[0]?.url} alt={bookingData.name} style={{ width: '100%', height: 'auto' }} />
                </Box>
                <Typography variant="h5" style={{ fontWeight: 'bold' }} gutterBottom>{bookingData.name}</Typography>
                <Typography variant="body2" color="textSecondary" gutterBottom>Mã tour: {bookingData.id}</Typography>
                <Divider sx={{ my: 1 }} />
                <SummaryItem>
                ...
                <Divider sx={{ my: 1 }} />
                <SummarySubtitle variant="subtitle2">KHÁCH HÀNG + PHỤ THU</SummarySubtitle>
                <SummaryItem>
                ...
                <SummaryItem>
                  <Typography variant="body2">Trẻ nhỏ:</Typography>
                ...
                <SummaryItem>
                  ...
                  <Typography variant="body2">{formData.babies} x {bookingData.bookingInfo?.babyPrice?.toLocaleString()} đ</Typography>
                </SummaryItem>
                <SummaryItem>
                  <Typography variant="body2">Phụ thu phòng đơn:</Typography>
                  <Typography variant="body2">{bookingData.bookingInfo?.singleRoomSurcharge?.toLocaleString()} đ</Typography>
                </SummaryItem>
                <Divider sx={{ my: 1 }} />
                <SummarySubtitle variant="subtitle2">ƯU ĐÃI ĐẶT ONLINE</SummarySubtitle>
                <SummaryItem>
                  <Typography variant="body2">Người lớn:</Typography>
                  <Typography variant="body2">1 x {bookingData.bookingInfo?.adultOnlineDiscount?.toLocaleString()} đ</Typography>
                </SummaryItem>
                <SummaryItem>
                  <Typography variant="body2">Trẻ em:</Typography>
                  <Typography variant="body2">1 x {bookingData.bookingInfo?.childOnlineDiscount?.toLocaleString()} đ</Typography>
                </SummaryItem>
                <Divider sx={{ my: 1 }} />
                <TotalPrice variant="h6">Tổng tiền: {calculateTotal().toLocaleString()} đ</TotalPrice>
